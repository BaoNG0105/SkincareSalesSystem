...
import { getRoutinesByTestResult } from "../../services/api.skinTest";
...
const Routine = () => {
    ...
    useEffect(() => {
        const fetchRoutineData = async () => {
            try {
                const data = await getRoutinesByTestResult(skinTypeId);
                setRoutineData(data);
            } catch (error) {
                toast.error("Can not load routine data.");
            } finally {
                setLoading(false);
            }
        };
                ...
                {routineData ? (
                    <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-8">
                        {routineData.map((step) => (
                            <div key={step.routineId} className="mb-12 last:mb-0">
